...
abstract public class TMUberService implements Comparable <TMUberService>
{
  ...
  {
    // compares distance based off which is greater, or checks if they are both equal
  ...
  {
    // Fill in the code
    TMUberService other2 = (TMUberService) other;     // type casts object other to tmuberservice

    if(!(this.user.equals(other2.getUser())))         // checks if users are not the same
    {
      return false;
    }

    if(!(this.type.equals(other2.type)))              // checks if types are not the same
    {
      return false;
    }
